...
# shp.uk <- readOGR(in_path, layer = 'UK')
shp.uk <- ms_simplify(shp.uk, keep = 0.25)

# save new smaller polygons as OA in 's00' for further aggregations and mapping
if(file.exists(file.path(out_path, 'OA.shp') ) ) 
    file.remove(paste0(out_path, '/OA.', c('shp', 'prj', 'dbf', 'shx')))
writeOGR(shp.uk, dsn = out_path, layer = 'OA', driver = 'ESRI Shapefile')
